	...
	<body style="margin:0px;font-family: 'Ubuntu', sans-serif; font-size: 13px; font-weight: 400; line-height: 23px;">
		...
		<table width="700" align="center" cellspacing="0" cellpadding="0" border="0" style="border:2px solid #444" id = "DivIdToPrint">
			<tr>
				...
				<strong style="font-size: 18px; text-transform: uppercase;">{{ $bookingslist[0]->users_bookings->role_id != 3 ? $bookingslist[0]->users_bookings->user_details->agentname : 'Travel Trip Plus'}}</strong><br>
				{{ $bookingslist[0]->users_bookings->role_id != 3 ? $bookingslist[0]->users_bookings->user_details->agentadd :($getsitedata['4']->value) }}<br>
				Contact No. : {{ $bookingslist[0]->users_bookings->role_id != 3 ? $bookingslist[0]->users_bookings->mobile : ($getsitedata['2']->value)  }}<br>
				Email Id: {{ $bookingslist[0]->users_bookings->role_id != 3 ?  $bookingslist[0]->users_bookings->email :($getsitedata['5']->value) }}
				</td>
			...
			<tr>
				<td colspan="2" style="padding:10px; ">
					<table width="700" cellpadding="0" cellspacing="0" border="0" style=" padding:10px 0px; line-height:20px;border-top:2px solid #ccc; border-bottom: 2px solid #ccc;">
						<tr>
							<td valign="top">
								Customer Contact No. : <strong>{{ $bookingslist[0]->users_bookings->mobile }}</strong><br>
					...
					<table width="700" cellspacing="0" cellpadding="0" border="0" style="padding: 10px; border:2px solid #ccc; ">
						...
						<td>{{$k == 0 ? 'Departure' : 'Return'}}</td>
							<td valign="top" style="padding-bottom:20px">
								...
								Operated by <strong>{{$bookdata->Airline->AirlineCode}}</strong>
							</td>
			...
			<tr>
				...
				<p><strong>"THANK YOU FOR BOOKING WITH {{ $bookingslist[0]->users_bookings->role_id != 3 ? $bookingslist[0]->users_bookings->user_details->agentname : 'Travel Trip Plus'}}"</strong></p>
				</td>
			</tr>
		</table>
		...
		// Get the <span> element that closes the modal
          ...
          var newUrl = oldUrl+'?kt='+$('#addmarkup').text(); 
